...
// returns a 201 created code
const created = (request, response, isXML) => {
  const content = {
    message: 'New resource created successfully.',
  };
  
  sendResponse(request, response, isXML, 201, content);
};

// returns a 204 No Content code
const noContent = (request, response, isXML) => {
  const content = {
    message: 'No content.',
  };
  
  sendResponse(request, response, isXML, 204, content);
};

// returns a 304 Not Modified code
const notModified = (request, response, isXML) => {
  const content = {
    message: 'Nothing was modified.',
  };
  
  sendResponse(request, response, isXML, 304, content);
};

// returns a 400 Bad Request code
const badRequest = (request, response, isXML) => {
  const content = {
    message: 'Invalid Request.',
  };
  
  sendResponse(request, response, isXML, 400, content);
};
